...
const DEFAULT_LOGO =
  'data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2240%22%20height%3D%2240%22%3E%3Crect%20width%3D%2240%22%20height%3D%2240%22%20fill%3D%22%23ddd%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%20fill%3D%22%23999%22%3E%3F%3C%2Ftext%3E%3C%2Fsvg%3E';
const DEFAULT_LOGO_LARGE =
  'data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2280%22%20height%3D%2280%22%3E%3Crect%20width%3D%2280%22%20height%3D%2280%22%20fill%3D%22%23ddd%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%20fill%3D%22%23999%22%3E%3F%3C%2Ftext%3E%3C%2Fsvg%3E';
...
function initializeEventListeners() {
  ...
  document
    .getElementById('selectAllIptvOrgCheckbox')
    .addEventListener('change', handleSelectAllIptvOrg);
...
function handleSearch(e) {
  ...
  const filtered = channels.filter(
    (c) =>
      c.channelName.toLowerCase().includes(query) || c.channelGroup.toLowerCase().includes(query),
  );
...
async function handleBulkTest() {
  ...
  if (channelsToTest.length > 100) {
    showToast(
      `Testing limited to 100 channels. You selected ${channelsToTest.length}, testing first 100.`,
      'warning',
    );
  ...
  showToast(
    `Tested ${data.tested} channels - ${workingCount} working, ${data.tested - workingCount} failed`,
    'success',
  );
  loadChannels();
}
...
async function fetchIptvOrgPlaylist(url) {
  try {
    ...
    const response = await fetch(
      `${API_BASE}/api/v1/iptv-org/fetch?url=${encodeURIComponent(url)}`,
      {
        headers: { 'X-Session-Id': sessionId },
      },
    );
...
async function handleImportSelected() {
  ...
  const selectedChannelsData = Array.from(selectedIptvOrgChannels).map(
    (index) => iptvOrgChannels[index],
  );
...
async function testChannels(mode) {
  ...
  if (
    !confirm(
      `Test ${channelsToTest.length} channels? This may take several minutes.${limitMessage}`,
    )
  )
    return;
  ...
  try {
    ...
    showToast(
      `Tested: ${data.tested} - Working: ${data.working}, Not Working: ${data.notWorking}`,
      'success',
    );
    loadChannels();
    loadTestResults();
...
function playChannel(channel) {
  ...
  if (Hls.isSupported()) {
      ...
      error: (e) => {
        ...
        if (video.error) {
          ...
          errorDetails.textContent =
            e.message || 'Failed to load HLS stream. The stream may be offline or incompatible.';
        }
      },
      loadedmetadata: () => {
    ...
    errorBox.classList.remove('hidden');
    errorDetails.textContent =
      'Your browser does not support HLS streaming. Please try using Chrome, Firefox, or Safari.';
  }
...
function renderApkVersionsTable() {
  ...
  if (apkVersions.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;padding:40px;">No APK versions uploaded yet</td></tr>';
    return;
  }

  tableBody.innerHTML = apkVersions
    .map(
      (version) => `
        <tr>
            ...
            <td>
                ...
                <span style="color: #6b7280; font-size: 0.75rem; margin-left: 0.5rem;">
                    (Version is managed via GitHub release tags)
                </span>
            </td>
        </tr>
    `,
...
async function deleteApkVersion(versionId) {
  if (
    !confirm('Are you sure you want to delete this version? This will also delete the APK file.')
  ) {
    return;
  }
