...
router.get('/api/channels', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to fetch channels from IPTV-org API'
        });
    }
});
...
router.get('/api/streams', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to fetch streams from IPTV-org API'
        });
    }
});
...
router.get('/api/enriched', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to fetch enriched channels'
        });
    }
});
...
router.get('/api/languages', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to fetch languages from IPTV-org API'
        });
    }
});
...
router.post('/import', async (req, res) => {
    try {
        ...
        if (replaceExisting) {
            // Use a session for atomic replace: delete + insert together
